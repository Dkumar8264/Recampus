  ...
  if (isLoading) {
    return (
      <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="space-y-4 rounded-[28px] border border-stone-200 bg-white/90 p-5 shadow-sm">
  ...
  if (!listing) {
    return (
      <section>
        ...
        <p className="mt-6 text-stone-600">Listing not found.</p>
      </section>
    );
  }
  ...
  return (
    <section>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
        <div className="overflow-hidden rounded-[28px] border border-stone-200 bg-white shadow-[0_24px_70px_rgba(64,43,20,0.12)]">
          <div className="flex aspect-[4/3] items-center justify-center bg-gradient-to-br from-white via-[#f7f1e8] to-[#e9f5ff]">
            {listing.images?.[0] ? (
        ...
        <div className="lg:sticky lg:top-24">
          <div className="rounded-[28px] border border-stone-200 bg-white/90 p-6 text-ink shadow-[0_24px_70px_rgba(64,43,20,0.12)] backdrop-blur">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 rounded-full border border-stone-200 bg-white px-3 py-1 text-xs font-bold text-stone-700">
                <MapPin size={13} aria-hidden="true" />
                {formatLabel(listing.location)}
              </span>
            </div>

            <h1 className="mt-5 font-display text-4xl font-black leading-tight text-ink">{listing.title}</h1>
            {listing.type === 'sale' ? (
              <p className="mt-3 text-3xl font-black text-ink">₹{listing.price}</p>
            ) : null}
